...
// Static workflow states (places). Group drives node color.
...
// Directed transitions. Self-loop is used for product-research retry.
...
export function WorkflowPage() {
	...
	useEffect(() => {
		...
		const labelGroup = zoomGroup.append("g").selectAll("g").data(links).enter().append("g");
		...
		const fitToView = () => {
			const padding = 120;
			const xs = nodes.map((n: D3NodeDatum) => n.x || 0);
			const ys = nodes.map((n: D3NodeDatum) => n.y || 0);

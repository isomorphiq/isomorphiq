...
import type { CreateUserInput, UserRole } from '../src/types.js'
...
async function runAuthTests() {
  ...
  try {
    ...
    console.log(`   Password "${strongPassword}": ${strongValidation.isValid ? '✅ Valid' : '❌ Invalid'}`)
    console.log()
    ...
    for (const testCase of testCases) {
      ...
      console.log(`   ${testCase.resource}:${testCase.action} - ${hasPermission ? '✅ Granted' : '❌ Denied'}`)
    }
    ...
    const _auth1 = await userManager.authenticateUser({ username: 'testuser', password: 'SecurePass123!' })
    const _auth2 = await userManager.authenticateUser({ username: 'testuser', password: 'SecurePass123!' })
    ...
    userAgents.forEach((ua, index) => {
      ...
      console.log(`   Device ${index + 1}: ${deviceInfo.type} - ${deviceInfo.os} ${deviceInfo.browser}`)
    })

...
class RealTimeUpdatesTester {
    ...
    async sendTRPCSubscription(ws, procedure) {
        const message = {
            type: 'subscription',
            payload: {
                path: procedure,
    ...
    async waitForMessage(ws, timeout = 5000) {
        return new Promise((resolve, reject) => {
    ...
    async testRealTimeTaskCreation() {
    ...
    async testSubscriptionFiltering() {
        await this.runTest('Subscription event filtering', async () => {
            ...
            try {
                ...
                if (createdEvents.length < 3) {
                    throw new Error(`Expected at least 3 TASK_CREATED events, received ${createdEvents.length}`);
                }
                // Cleanup test tasks
...
//# sourceMappingURL=test-real-time-updates.js.map
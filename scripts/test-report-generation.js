...
class ReportGeneratorTester {
    ...
    async generateReportData() {
        ...
        const analytics = {
            ...
            performance: {
                ...
                productivityScore: totalTasks > 0 ? `${Math.min(100, Math.round((completedTasks / totalTasks) * 100))}%` : '0%',
    ...
    async testJSONReportGeneration() {
        await this.runTest('JSON report generation', async () => {
            ...
            if (!reportData.summary || !reportData.tasks || !reportData.analytics || !reportData.timeline) {
            ...
            if (typeof reportData.summary.completionRate !== 'number' ||
                reportData.summary.completionRate < 0 || reportData.summary.completionRate > 100) {
                throw new Error('Invalid completion rate in summary');
            }
            // Validate analytics
            if (!reportData.analytics.overview || !reportData.analytics.priority || !reportData.analytics.performance) {
            ...
            const timelineHasValidData = reportData.timeline.every(day => day.date &&
    ...
    async testCSVReportGeneration() {
        await this.runTest('CSV report generation', async () => {
            const reportData = await this.generateReportData();
            // Generate CSV content
            const csvHeaders = ['Date', 'Tasks Created', 'Tasks Completed', 'Active Tasks'];
            const csvRows = reportData.timeline.map(day => [day.date, day.created, day.completed, day.active].join(','));
    ...
    async testHTMLReportGeneration() {
        await this.runTest('HTML report generation', async () => {
            ...
            if (!htmlContent.includes('Summary') || !htmlContent.includes('Analytics') || !htmlContent.includes('Timeline')) {
...
<body>
    ...
    async testPDFReportGeneration() {
        await this.runTest('PDF report structure validation', async () => {
            ...
            if (!Array.isArray(timelineSection.table.headers) || timelineSection.table.headers.length !== 4) {
    ...
    async testReportAggregation() {
        await this.runTest('Report data aggregation accuracy', async () => {
            ...
            if (reportData.summary.totalTasks !== actualTotal) {
                throw new Error(`Total tasks aggregation mismatch: expected ${actualTotal}, got ${reportData.summary.totalTasks}`);
            }
            // Check analytics aggregation
            if (reportData.analytics.overview.totalTasks !== actualTotal) {
                throw new Error(`Analytics total tasks mismatch: expected ${actualTotal}, got ${reportData.analytics.overview.totalTasks}`);
            }
            if (reportData.analytics.overview.completedTasks !== actualCompleted) {
                throw new Error(`Completed tasks aggregation mismatch: expected ${actualCompleted}, got ${reportData.analytics.overview.completedTasks}`);
            }
            if (reportData.analytics.overview.inProgressTasks !== actualInProgress) {
                throw new Error(`In-progress tasks aggregation mismatch: expected ${actualInProgress}, got ${reportData.analytics.overview.inProgressTasks}`);
            }
            if (reportData.analytics.overview.todoTasks !== actualTodo) {
                throw new Error(`Todo tasks aggregation mismatch: expected ${actualTodo}, got ${reportData.analytics.overview.todoTasks}`);
            }
            if (reportData.analytics.priority.high !== actualHigh) {
                throw new Error(`High priority aggregation mismatch: expected ${actualHigh}, got ${reportData.analytics.priority.high}`);
            }
            if (reportData.analytics.priority.medium !== actualMedium) {
                throw new Error(`Medium priority aggregation mismatch: expected ${actualMedium}, got ${reportData.analytics.priority.medium}`);
            }
            if (reportData.analytics.priority.low !== actualLow) {
                throw new Error(`Low priority aggregation mismatch: expected ${actualLow}, got ${reportData.analytics.priority.low}`);
            }
            // Verify completion rate calculation
            const expectedCompletionRate = actualTotal > 0 ? Math.round((actualCompleted / actualTotal) * 100) : 0;
            if (reportData.analytics.overview.completionRate !== expectedCompletionRate) {
                throw new Error(`Completion rate calculation mismatch: expected ${expectedCompletionRate}%, got ${reportData.analytics.overview.completionRate}%`);
            }
        });
    }
    async testReportFileOutput() {

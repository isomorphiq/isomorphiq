    ...
    async getTestData() {
        ...
        const analytics = {
            ...
            performance: {
                ...
                productivityScore: totalTasks > 0 ? `${Math.min(100, Math.round((completedTasks / totalTasks) * 100 + (todayCompleted * 10)))}%` : '0%',
    ...
    async testAnalyticsAccuracy() {
        await this.runTest('Analytics total tasks accuracy', async () => {
            ...
            if (analytics.overview.totalTasks !== tasks.length) {
                throw new Error(`Expected ${tasks.length} total tasks, got ${analytics.overview.totalTasks}`);
            }
        });
        await this.runTest('Analytics status breakdown accuracy', async () => {
            ...
            if (analytics.overview.completedTasks !== actualDone) {
                throw new Error(`Completed tasks mismatch: expected ${actualDone}, got ${analytics.overview.completedTasks}`);
            }
            if (analytics.overview.inProgressTasks !== actualInProgress) {
                throw new Error(`In-progress tasks mismatch: expected ${actualInProgress}, got ${analytics.overview.inProgressTasks}`);
            }
            if (analytics.overview.todoTasks !== actualTodo) {
                throw new Error(`Todo tasks mismatch: expected ${actualTodo}, got ${analytics.overview.todoTasks}`);
            }
        });
        await this.runTest('Analytics completion rate accuracy', async () => {
            ...
            if (analytics.overview.completionRate !== expectedRate) {
                throw new Error(`Completion rate mismatch: expected ${expectedRate}%, got ${analytics.overview.completionRate}%`);
            }
        });
        await this.runTest('Analytics priority breakdown accuracy', async () => {
            ...
            if (analytics.priority.high !== actualHigh) {
                throw new Error(`High priority tasks mismatch: expected ${actualHigh}, got ${analytics.priority.high}`);
            }
            if (analytics.priority.medium !== actualMedium) {
                throw new Error(`Medium priority tasks mismatch: expected ${actualMedium}, got ${analytics.priority.medium}`);
            }
            if (analytics.priority.low !== actualLow) {
                throw new Error(`Low priority tasks mismatch: expected ${actualLow}, got ${analytics.priority.low}`);
            }
        });
        await this.runTest('Analytics today stats accuracy', async () => {
            ...
            if (analytics.today.created !== todayCreated) {
                throw new Error(`Today created tasks mismatch: expected ${todayCreated}, got ${analytics.today.created}`);
            }
            if (analytics.today.completed !== todayCompleted) {
                throw new Error(`Today completed tasks mismatch: expected ${todayCompleted}, got ${analytics.today.completed}`);
            }
        });
    }
    async testStatsConsistency() {
        await this.runTest('Stats vs Analytics consistency', async () => {
            ...
            if (stats.total !== analytics.overview.totalTasks) {
                throw new Error(`Total tasks inconsistency: stats=${stats.total}, analytics=${analytics.overview.totalTasks}`);
            }
            if (stats.byStatus.done !== analytics.overview.completedTasks) {
                throw new Error(`Completed tasks inconsistency: stats=${stats.byStatus.done}, analytics=${analytics.overview.completedTasks}`);
            }
            if (stats.byStatus['in-progress'] !== analytics.overview.inProgressTasks) {
                throw new Error(`In-progress tasks inconsistency: stats=${stats.byStatus['in-progress']}, analytics=${analytics.overview.inProgressTasks}`);
            }
            if (stats.byStatus.todo !== analytics.overview.todoTasks) {
                throw new Error(`Todo tasks inconsistency: stats=${stats.byStatus.todo}, analytics=${analytics.overview.todoTasks}`);
            }
            if (stats.byPriority.high !== analytics.priority.high) {
                throw new Error(`High priority inconsistency: stats=${stats.byPriority.high}, analytics=${analytics.priority.high}`);
            }
            if (stats.byPriority.medium !== analytics.priority.medium) {
                throw new Error(`Medium priority inconsistency: stats=${stats.byPriority.medium}, analytics=${analytics.priority.medium}`);
            }
            if (stats.byPriority.low !== analytics.priority.low) {
                throw new Error(`Low priority inconsistency: stats=${stats.byPriority.low}, analytics=${analytics.priority.low}`);
            }
        });
    }
        ...
        await this.runTest('Timeline data accuracy', async () => {
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                if (dayCreated < 0 || dayCompleted < 0) {
                    throw new Error(`Invalid timeline counts for ${date.toISOString().split('T')[0]}: created=${dayCreated}, completed=${dayCompleted}`);
                }
            }
        });
    }
    async testPerformanceMetrics() {
        await this.runTest('Performance metrics validity', async () => {
            ...
            if (totalActiveTasks !== expectedActive) {
                throw new Error(`Active tasks mismatch: expected ${expectedActive}, got ${totalActiveTasks}`);
            }
        });
    }
    async testEdgeCases() {
        ...
        await this.runTest('Single task scenarios', async () => {
            ...
            for (const scenario of scenarios) {
                ...
                if (scenario.status === 'done' && completionRate !== 100) {
                    throw new Error(`Single done task should have 100% completion rate, got ${completionRate}%`);
                }
                if (scenario.status !== 'done' && completionRate !== 0) {
                    throw new Error(`Single non-done task should have 0% completion rate, got ${completionRate}%`);
                }
            }
        });
    }
...
//# sourceMappingURL=test-analytics-dashboard.js.map
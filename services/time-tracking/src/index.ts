...
export type TimeUtilizationReport = {
    message: string;
};
...
export type ITimeTrackingService = {
...
export class TimeTrackingService implements ITimeTrackingService {
	...
	async startTimeEntry(input: TimeEntryCreateInput, createdBy: string): Promise<Result<TimeEntry>> {
		...
		const timerInput: TimeEntryCreateInput = { ...input, type: "timer" };
	...
	async createManualTimeEntry(
		...
		const manualInput: TimeEntryCreateInput = { ...input, type: "manual" };

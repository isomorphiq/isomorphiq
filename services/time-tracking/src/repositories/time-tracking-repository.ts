...
import type { TimeEntry, TimeSearchOptions, Timesheet } from "@isomorphiq/time-tracking";

/**
 * Repository interface for Time Tracking data access
 */
/* eslint-disable no-unused-vars */
export type ITimeTrackingRepository = {
	...
	async searchTimeEntries(
		...
		try {
			...
			if (options.filters) {
				...
				if (dateFrom) {
					entries = entries.filter((entry) => new Date(entry.startTime) >= dateFrom);
				}
				if (dateTo) {
					entries = entries.filter((entry) => new Date(entry.startTime) <= dateTo);
				}
			...
			if (options.sort) {
				...
				const resolveSortValue = (entry: TimeEntry): number => {
					switch (field) {
						case "startTime":
							return entry.startTime.getTime();
						case "duration":

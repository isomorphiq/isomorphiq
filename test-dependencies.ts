...
async function testDependencyProcessing() {
  ...
  try {
    ...
    const task1 = await pm.createTask(
      ...
      'Create the initial database schema for user management',
      'high',
      []
    )
    ...
    const task3 = await pm.createTask(
      ...
      'Build the UI for user profile management',
      'medium',
      [task2.id]
    )
    ...
    const highPriorityTask = await pm.createTask(
      'High priority independent task',
      'This should come before other independent tasks',
      'high',
      []
    )

...
export class PluginManager extends EventEmitter implements IPluginManager {
	...
	async loadPluginsFromDirectory(directory: string): Promise<void> {
		try {
			...
			const pluginFiles = await this.loader.discoverPlugins(directory);
			const loadedPlugins: string[] = [];
			const failedPlugins: Array<{ file: string; error: Error }> = [];

			for (const filePath of pluginFiles) {
				try {
					const plugin = await this.loader.loadPlugin(filePath);
					await this.registerPlugin(plugin);
					loadedPlugins.push(plugin.metadata.name);
				} catch (error) {
					failedPlugins.push({ file: filePath, error: error as Error });
					console.error(`[PLUGIN-MANAGER] Failed to load plugin from ${filePath}:`, error);
				}
			}
	...
	async reloadPlugin(name: string): Promise<void> {
		...
		try {
			...
			// Reload plugin
			const reloadedPlugin = await this.loader.loadPlugin(pluginPath, entry.config);
			await this.registerPlugin(reloadedPlugin, entry.config);
	...
	private async getPluginPath(name: string): Promise<string | null> {
		...
		try {
			const files = await this.loader.discoverPlugins(this.pluginsDirectory);
			for (const file of files) {
				const plugin = await this.loader.loadPlugin(file);
				if (plugin.metadata.name === name) {
					await plugin.cleanup(); // Clean up the test instance
					return file;
				}
			}
		} catch {

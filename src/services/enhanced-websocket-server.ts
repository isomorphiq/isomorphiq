...
export class ConnectionPool {
	...
	private connectionHistory: Array<{ id: string; connectedAt: Date; disconnectedAt?: Date }> = [];
	...
	addConnection(client: WebSocketClient): boolean {
		...
		console.log(`[WS] Client connected: ${client.id} (Total: ${this.connections.size})`);
		return true;
	}
	...
	removeConnection(clientId: string): WebSocketClient | null {
		...
		if (client) {
			...
			// Update history
			const historyEntry = this.connectionHistory.find((h) => h.id === clientId);
			if (historyEntry) {
				historyEntry.disconnectedAt = new Date();
			}

			console.log(`[WS] Client disconnected: ${clientId} (Total: ${this.connections.size})`);
		}
		return client || null;
	}
	...
	getConnectionsBySubscription(eventType: WebSocketEventType): WebSocketClient[] {
	...
	getStats(): {
		...
		const recentConnections = this.connectionHistory.filter((h) => h.connectedAt > oneHourAgo);
		const connectionsWithDuration = this.connectionHistory.filter((h) => h.disconnectedAt);

		const totalDuration = connectionsWithDuration.reduce((sum, h) => {
			return sum + (h.disconnectedAt?.getTime() - h.connectedAt.getTime());
		}, 0);

		const averageConnectionTime =
			connectionsWithDuration.length > 0 ? totalDuration / connectionsWithDuration.length : 0;
	...
	cleanup(): void {
		...
		for (const [clientId, client] of this.connections) {
			...
			if (timeSinceLastPing > staleThreshold || client.socket.readyState !== WebSocket.OPEN) {
				console.log(`[WS] Cleaning up stale connection: ${clientId}`);
				try {
					client.socket.close(1000, "Stale connection");
				} catch (error) {
					console.error(`[WS] Error closing stale connection ${clientId}:`, error);
				}
...
export class MessageQueue {
	private queue: Array<{ event: WebSocketEvent; timestamp: number; retries: number }> = [];
	...
	enqueue(event: WebSocketEvent): boolean {
		if (this.queue.length >= this.maxSize) {
			console.warn(`[WS] Message queue full, dropping message: ${event.type}`);
			return false;
		}
	...
	requeue(event: WebSocketEvent, currentRetries: number): boolean {
		if (currentRetries >= this.maxRetries) {
			console.warn(`[WS] Max retries exceeded for message: ${event.type}`);
			return false;
		}
...
export class EnhancedWebSocketManager {
	...
	async stop(): Promise<void> {
		...
		for (const client of connections) {
			try {
				...
				console.error(`[WS] Error closing connection ${client.id}:`, error);
			}
		}
	...
	private handleConnection(ws: WebSocket, _req: IncomingMessage): void {
		...
		if (this.config.enableCompression) {
			...
			ws.send(JSON.stringify({ type: "compression_enabled", enabled: true }));
		}
	}
	...
	broadcast(event: WebSocketEvent): void {
		const targetClients = this.connectionPool.getConnectionsBySubscription(event.type);
		...
		if (targetClients.length < this.connectionPool.getAllConnections().length) {
			...
			for (const client of connections) {
				if (client.socket.readyState === WebSocket.OPEN) {
					try {
						...
						console.error(`[WS] Ping failed for client ${client.id}:`, error);
	...
	getClientsInfo(): Array<{
		...
		return this.connectionPool.getAllConnections().map((client) => ({
			...
			connectionDuration: Math.floor((now.getTime() - client.lastPing.getTime()) / 1000),
		}));
	}
}

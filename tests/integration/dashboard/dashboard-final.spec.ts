...
import { canReachDashboard } from "../../e2e/dashboard/test-environment.ts";
...
before(async () => {
    dashboardReachable = await canReachDashboard();
});

beforeEach(() => {
    test.skip(!dashboardReachable, "Dashboard server is not reachable at http://localhost:3005");
});

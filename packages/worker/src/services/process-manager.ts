	...
	async initialize(): Promise<void> {
        ...
        this.workflowRunner = new ProfileWorkflowRunner({
            ...
            appendTaskActionLogEntry: async (taskId, entry, fallbackLog) => {
                ...
                const task = await this.productManager.getTask(taskId);
                const currentLog = task?.actionLog ?? fallbackLog ?? [];
                await this.productManager.updateTask(
                    taskId,
	...
	private async handleCommand(message: string, socket: Socket): Promise<void> {
		try {
			...
			switch (command.command) {
				...
				case "cancel_task":
					if (!this.productManager) throw new Error("ProductManager not initialized");
					const oldTaskCancel = await this.productManager
						.getAllTasks()
						.then((t) => t.find((task) => task.id === command.data.id));
				...
				case "retry_task":
					if (!this.productManager) throw new Error("ProductManager not initialized");
					const oldTaskRetry = await this.productManager
						.getAllTasks()
						.then((t) => t.find((task) => task.id === command.data.id));
					const oldStatusRetry = oldTaskRetry?.status || "failed";
					const retriedTask = await this.productManager.updateTaskStatus(
						command.data.id,
						"todo",
					);
					result = retriedTask;

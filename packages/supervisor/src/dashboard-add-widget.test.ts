// FILE_CONTEXT: "context-82038d6a-6f3b-4273-8a1c-9ba2f0a32fe1"
...
test("uses requested placement and resolves collisions when adding from library", async () => {
    const storage = createInMemoryDashboardStorage();
    await addWidgetFromLibrary({
        storage,
        library: sampleLibrary,
    ...
    assert.equal(result.type, "added");
    if (result.type === "added") {
        assert.equal(result.widget.placement.x, 4);
        assert.equal(result.widget.placement.y, 0);
    }
});

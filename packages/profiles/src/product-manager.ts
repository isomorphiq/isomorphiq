// TODO: This file is too complex (1151 lines) and should be refactored into several modules.
// Current concerns mixed: Task CRUD operations, search functionality, automation rules,
// template management, persistence layer abstraction, WebSocket event handling.
// 
// Proposed structure:
// - tasks/product-manager/index.ts - Main ProductManager class composition
// - tasks/product-manager/crud-service.ts - Task create, read, update, delete operations
// - tasks/product-manager/search-service.ts - Task search, filtering, and saved searches
// - tasks/product-manager/automation-service.ts - Automation rule engine execution
// - tasks/product-manager/template-service.ts - Template management and creation
// - tasks/product-manager/persistence/ - Repository implementations (LevelDB, memory)
// - tasks/product-manager/event-service.ts - WebSocket event publishing
// - tasks/product-manager/types.ts - ProductManager-specific types

import { existsSync } from "node:fs";
import path from "node:path";
import { ConfigManager, globalEventBus, resolveEnvironmentValue } from "@isomorphiq/core";
...
export type ProductManagerOptions = {
    ...
    taskServiceWsUrl?: string;
    enableTaskSubscriptions?: boolean;
...
const resolveTaskEnvironment = (options: ProductManagerOptions): string => {
    const envConfig = ConfigManager.getInstance().getEnvironmentConfig();
    const raw = options.environment ?? process.env.ISOMORPHIQ_ENVIRONMENT;
    return resolveEnvironmentValue(raw, envConfig);
};

const resolveTaskResult = <T>(result: { success: boolean; data?: T; error?: Error }): T => {
    if (!result.success) {
        throw result.error ?? new Error("Task service error");
    }
    return result.data as T;
};

const createLocalTaskClient = (taskService: TaskServiceApi): TaskClient => ({
    listTasks: async () => resolveTaskResult(await taskService.getAllTasks()),
    ...
    createTask: async (input: CreateTaskInputWithPriority, createdBy?: string) =>
        resolveTaskResult(await taskService.createTask(input, createdBy ?? "system")),
    updateTask: async (id: string, updates: Omit<ExtendedUpdateTaskInput, "id">, updatedBy?: string) =>
        resolveTaskResult(
            await taskService.updateTask(id, { id, ...updates }, updatedBy ?? "system"),
        ),
    deleteTask: async (id: string, deletedBy?: string) => {
        await taskService.deleteTask(id, deletedBy ?? "system");
    },
    updateTaskStatus: async (id: string, status: TaskStatus, updatedBy?: string) =>
        resolveTaskResult(await taskService.updateTaskStatus(id, status, updatedBy ?? "system")),
    claimTask: async (id: string, workerId: string) => {
        ...
        const claimed = await taskService.updateTaskStatus(id, "in-progress", workerId);
        return claimed.success ? claimed.data : null;
    },
    updateTaskPriority: async (id: string, priority: TaskPriority, updatedBy?: string) =>
        resolveTaskResult(await taskService.updateTaskPriority(id, priority, updatedBy ?? "system")),
    assignTask: async (id: string, assignedTo: string, assignedBy?: string) =>
        resolveTaskResult(await taskService.assignTask(id, assignedTo, assignedBy ?? "system")),
    addCollaborator: async (id: string, userId: string, updatedBy?: string) =>
        resolveTaskResult(await taskService.addCollaborator(id, userId, updatedBy ?? "system")),
    removeCollaborator: async (id: string, userId: string, updatedBy?: string) =>
        resolveTaskResult(await taskService.removeCollaborator(id, userId, updatedBy ?? "system")),
    addWatcher: async (id: string, userId: string, updatedBy?: string) =>
        resolveTaskResult(await taskService.addWatcher(id, userId, updatedBy ?? "system")),
    removeWatcher: async (id: string, userId: string, updatedBy?: string) =>
        resolveTaskResult(await taskService.removeWatcher(id, userId, updatedBy ?? "system")),
    addDependency: async (id: string, dependsOn: string, updatedBy?: string) =>
        resolveTaskResult(await taskService.addDependency(id, dependsOn, updatedBy ?? "system")),
    removeDependency: async (id: string, dependsOn: string, updatedBy?: string) =>
        resolveTaskResult(await taskService.removeDependency(id, dependsOn, updatedBy ?? "system")),
    getTasksByStatus: async (status: TaskStatus) =>
        resolveTaskResult(await taskService.getTasksByStatus(status)),
    getTasksByUser: async (userId: string) =>
        resolveTaskResult(await taskService.getTasksByUser(userId)),
    getTasksSortedByDependencies: async () =>
        resolveTaskResult(await taskService.getTasksSortedByDependencies()),
    createManyTasks: async (inputs: CreateTaskInputWithPriority[], createdBy?: string) =>
        resolveTaskResult(await taskService.createManyTasks(inputs, createdBy ?? "system")),
    close: async () => {},
});
...
export class ProductManager {
    public taskService: TaskServiceApi;
    ...
    private taskEventSubscription: { unsubscribe: () => void } | null = null;
    ...
    constructor(dbPath?: string, options: ProductManagerOptions = {}) {
        ...
        const environment = resolveTaskEnvironment(options);
        const localTaskService = storageMode === "memory" && !options.taskClient
            ? new EnhancedTaskService(new InMemoryTaskRepository())
            : null;
        this.taskClient = options.taskClient
            ?? (localTaskService
                ? createLocalTaskClient(localTaskService)
                : createTaskClient({
                    environment,
                    url: options.taskServiceUrl,
                    wsUrl: options.taskServiceWsUrl,
                    enableSubscriptions: options.enableTaskSubscriptions,
                }));
        this.taskService = localTaskService ?? createTaskServiceClient(this.taskClient);
	...
	async initialize(): Promise<void> {
		...
		try {
            ...
            this.ensureTaskEventSubscription();
    ...
    private ensureTaskEventSubscription(): void {
        if (this.taskEventSubscription || !this.taskClient.subscribeToTaskEvents) {
            return;
        }

        this.taskEventSubscription = this.taskClient.subscribeToTaskEvents((event) => {
    ...
    private handleTaskEvent(event: TaskEvent): void {
        ...
        const updatedBy = event.updatedBy ?? "system";
        const task = event.task ? this.convertTaskEntityToTask(event.task) : undefined;

        switch (event.type) {
            case "task_created":
                if (task) {
                    this.wsManager.broadcastTaskCreated(task, updatedBy);
                }
                break;
            case "task_updated":
                if (task) {
                    this.wsManager.broadcastTaskUpdated(task, {}, updatedBy);
                }
                break;
            case "task_assigned":
                if (task && task.assignedTo) {
                    this.wsManager.broadcastTaskAssigned(task, task.assignedTo, updatedBy);
                }
                break;
            case "task_collaborators_changed":
                if (task && task.collaborators) {
                    this.wsManager.broadcastTaskCollaboratorsUpdated(task, task.collaborators, updatedBy);
                }
                break;
            case "task_watchers_changed":
                if (task && task.watchers) {
                    this.wsManager.broadcastTaskWatchersUpdated(task, task.watchers, updatedBy);
                }
                break;
            case "task_status_changed":
                if (task && event.newStatus) {
                    this.wsManager.broadcastTaskStatusChanged(
                        event.taskId ?? task.id,
                        event.oldStatus ?? task.status,
                        event.newStatus,
                        task,
                        updatedBy,
                    );
            ...
            case "task_priority_changed":
                if (task && event.newPriority) {
                    this.wsManager.broadcastTaskPriorityChanged(
                        event.taskId ?? task.id,
                        event.oldPriority ?? task.priority,
                        event.newPriority,
                        task,
                        updatedBy,
                    );
            ...
            case "task_deleted":
                if (event.taskId) {
                    this.wsManager.broadcastTaskDeleted(event.taskId, updatedBy);
                }
                break;
	...
	async cleanup(): Promise<void> {
		try {
            ...
            await this.savedSearchesDb.close();
            await this.integrationDb.close();
            await this.taskClient.close();
            console.log("[PRODUCT-MANAGER] Cleanup completed");
		} catch (error) {

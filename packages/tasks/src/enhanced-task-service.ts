...
export class EnhancedTaskService {
	...
	private handleTaskCreated(event: TaskCreatedEvent): void {
		...
		console.log(`[TaskService] Task created: ${taskId} by ${createdBy}`);
		// Could trigger additional business logic here
	}

	private handleTaskUpdated(event: TaskUpdatedEvent): void {
		const taskId = event.data.task.id;
		const updatedBy = event.data.updatedBy ?? "unknown";
		console.log(`[TaskService] Task updated: ${taskId} by ${updatedBy}`);
		// Could trigger additional business logic here
	}
	...
	private handleTaskDeleted(event: TaskDeletedEvent): void {
		...
		console.log(`[TaskService] Task deleted: ${taskId} by ${deletedBy}`);
		// Could trigger additional business logic here
	}

...
describe("Progress Tracking Service", () => {
	...
	before(async () => {
		// Initialize services
		testDbPath = await mkdtemp(path.join(tmpdir(), "isomorphiq-progress-audit-db-"));
	...
	it("should apply progress filters correctly", async () => {
        ...
        // Should only include high priority tasks in the selected statuses
        const validTasks = filteredTasks.filter(task => 
            task.priority === "high" && 
            ["done", "in-progress", "failed"].includes(task.status as string)
        );

...
describe("TaskAuditService", () => {
    ...
    describe("History Querying and Filtering", () => {
        beforeEach(async () => {
            ...
            const jan1Later = new Date("2024-01-01T01:00:00Z");
            const jan1LaterAgain = new Date("2024-01-01T02:00:00Z");
            const jan2 = new Date("2024-01-02T00:00:00Z");
            const jan2Later = new Date("2024-01-02T01:00:00Z");
            ...
            await auditService.recordTaskCreated({
                ...
                status: "todo",
                priority: "low",
                createdAt: jan1.toISOString(),
                updatedAt: jan1.toISOString(),
                createdBy: "user1",
            }, "user1", jan1);

            await auditService.recordTaskStatusChanged("filter-test-1", "todo", "in-progress", "user1", undefined, undefined, jan1Later);
            await auditService.recordTaskStatusChanged("filter-test-1", "in-progress", "done", "user2", undefined, undefined, jan1LaterAgain);
            ...
            await auditService.recordTaskCreated({
                ...
                status: "todo",
                priority: "high",
                createdAt: jan2.toISOString(),
                updatedAt: jan2.toISOString(),
                createdBy: "user2",
            }, "user2", jan2);

            await auditService.recordTaskPriorityChanged("filter-test-2", "high", "medium", "user1", jan2Later);
        });
        ...
        it("should filter by date range", async () => {
            const history = await auditService.getTaskHistory({
                ...
                toDate: new Date("2024-01-02T23:59:59.999Z")
            });
        ...
        it("should apply multiple filters", async () => {
            ...
            strict.equal(history.length, 2); // Created task and priority change by user1
        });
    ...
    describe("Task History Summary", () => {
        beforeEach(async () => {
            ...
            const status1Time = new Date(baseTime.getTime() + 5000);
            const status2Time = new Date(status1Time.getTime() + 3000);
            const status3Time = new Date(status2Time.getTime() + 1000);
            const status4Time = new Date(status3Time.getTime() + 2000);
            const status5Time = new Date(status4Time.getTime() + 4000);
            ...
            await auditService.recordTaskStatusChanged("summary-test-1", "todo", "in-progress", "test-user", undefined, 5000, status1Time);
            await auditService.recordTaskStatusChanged("summary-test-1", "in-progress", "failed", "test-user", "Failed execution", 3000, status2Time);
            await auditService.recordTaskStatusChanged("summary-test-1", "failed", "todo", "test-user", undefined, undefined, status3Time); // retry
            await auditService.recordTaskStatusChanged("summary-test-1", "todo", "in-progress", "test-user", undefined, 2000, status4Time);
            await auditService.recordTaskStatusChanged("summary-test-1", "in-progress", "done", "test-user", undefined, 4000, status5Time);
        });
    ...
    describe("Audit Statistics", () => {
        beforeEach(async () => {
            ...
            for (let i = 1; i <= 5; i++) {
                const eventDate = new Date(`2024-01-${i.toString().padStart(2, "0")}T00:00:00Z`);
                const statusDate = new Date(eventDate.getTime() + 60 * 60 * 1000);
                await auditService.recordTaskCreated({
                    id: `stats-test-${i}`,
                    title: `Stats Task ${i}`,
                ...
                if (i <= 3) {
                    await auditService.recordTaskStatusChanged(`stats-test-${i}`, "todo", "done", `user${i}`, undefined, undefined, statusDate);
                } else {
                    await auditService.recordTaskStatusChanged(`stats-test-${i}`, "todo", "failed", `user${i}`, undefined, undefined, statusDate);
                }
            }
        });
    ...
    describe("Database Maintenance", () => {
        it("should cleanup old events", async () => {
            const oldEventDate = new Date("2020-01-01T00:00:00Z");
            // Create old event
            await auditService.recordTaskCreated({
                id: "old-event-task",
                title: "Old Task",
                ...
                status: "todo",
                priority: "low",
                createdAt: oldEventDate.toISOString(),
                updatedAt: oldEventDate.toISOString(),
                createdBy: "old-user",
            }, "old-user", oldEventDate);

// TODO: This file is too complex (754 lines) and should be refactored into several modules.
// Current concerns mixed: Progress metrics calculation, status tracking, performance scoring,
// overdue detection, analytics aggregation, filtering and querying.
// 
// Proposed structure:
// - progress/progress-service.ts - Main progress tracking orchestration
// - progress/metrics-calculator.ts - Progress metrics computation
// - progress/status-tracker.ts - Task status transition tracking
// - progress/performance-scorer.ts - Performance score calculation
// - progress/overdue-detector.ts - Overdue task identification
// - progress/analytics-aggregator.ts - Progress analytics and reporting
// - progress/query-service.ts - Progress data filtering and querying
...
export class ProgressTrackingService {
	...
	private calculatePerformanceScore(task: Task, history: TaskAuditEvent[], currentAge: number, processingTime: number, created: Date, updated: Date): number {
		let score = 100;
		const effectiveAge = (task.status as string) === "done" && processingTime > 0
			? processingTime
			: currentAge;

		// Deduct points for being overdue
		const expectedTime = this.getExpectedCompletionTime(task);
		if (expectedTime && effectiveAge > expectedTime) {
			const overdueRatio = (effectiveAge - expectedTime) / expectedTime;

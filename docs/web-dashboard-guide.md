...
```bash
# Start all services
npm run daemon &      # Background task processing
npm run http-api &    # REST API server
npm run web:dev       # Development frontend
```
...
npm run daemon &      # Serves static files from ./dist/
npm run http-api &
```
